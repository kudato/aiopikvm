...
| `WebRTCError` | The Janus gateway or its ustreamer plugin refused, or the negotiation never completed. Everything under `/janus/ws` is Janus's own protocol, so this carries `code` and `reason` — Janus's numbering or the plugin's — instead of an HTTP status; `code` is `0` when the failure had none. It is **not** a `WebSocketError`: the signalling socket itself failing still raises that one |
