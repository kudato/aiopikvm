...
(browser forward) — the `MouseButton` type, shared with the REST call. It is
worth having here: a name kvmd does not know is dropped inside its handler
with no answer of any kind, exactly as a bad key name is.
